...
writer = SummaryWriter(log_dir='./runs/exp2/',comment='exp3')
dataset = 'Cora'
...
print(type(torch.arange(data.num_nodes)))
...
#图嵌入可视化
label = data.y
output = model(torch.arange(data.num_nodes, device=device))
writer.add_embedding(output, metadata=label)
